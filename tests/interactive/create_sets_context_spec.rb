...
RSpec.describe "create song/songbook devient le contexte courant" do
  ...
  describe "create songbook" do
    ...
    it "remplace un carnet déjà en contexte (use songbook) par le carnet nouvellement créé" do
      ...
      allow(prompt).to receive(:ask).with("Sous-titre (rien si aucun) :").and_return("")
      allow(prompt).to receive(:ask).with("Prix (ex. 9,90 € — rien si inconnu) :").and_return("")
      allow(prompt).to receive(:ask).with("Nom de l'éditeur (rien si aucun) :").and_return("")
      allow(prompt).to receive(:ask).with("Conception du carnet (rien si inconnu) :").and_return("")
      allow(prompt).to receive(:yes?).and_return(false)
