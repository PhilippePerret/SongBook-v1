...
RSpec.describe "assistant de création de carnet" do
  ...
  it "Créer un carnet avec les bonnes données" do
    ...
    allow(prompt).to receive(:ask).with(SongbookCreator.yellow("Conception du carnet (rien si inconnu) :")).and_return("Philippe")
  ...
  it "Retrouver un carnet déjà créé au lieu d'en refaire un" do
    ...
    allow(prompt).to receive(:ask).with(SongbookCreator.yellow("Titre du carnet :")).and_return("Peu importe")
    allow(prompt).to receive(:ask).with(SongbookCreator.yellow("Nom du dossier :"), default: "Carnet-peu-importe").and_return("Carnet-De-Test-Assistant")
    allow(prompt).to receive(:yes?).and_return(false)
