...
#
# PAS ENCORE CÂBLÉ au programme (attente fin de session de Claude 2 sur le
# builder, câblage prévu prochaine session). Validé à la main par Phil sur deux
# cas (Am → Cm, Am → C#m), voir tests en bas de fichier.
#
# TODO intégration (prochaine session, une fois discuté avec Phil comment
# gabarit_essai1.rb/build.rb doivent l'appeler) :
# - Lire `transpose: X → Y` dans les .infos (voir Chansons/À bicyclette/w.infos).
# - Appliquer Transpose.transpose_chord à chaque accord rencontré dans le .lyr.
# - Gestion de la case (fret) du diagramme, cas par cas déjà validés avec Phil :
#   - case explicite N -> cible = N + décalage (demi-tons) ;
#   - 0 <= cible <= 10 et diagramme dispo -> direct ;
#   - 0 <= cible <= 10 mais diagramme PAS ENCORE construit -> signaler (log ou
#     immédiat selon sensibilité) + accord "neutre" (case la plus basse dispo,
#     comportement actuel par défaut de `diag_path`) en attendant ;
#   - cible > 10 -> diagramme EXISTANT le plus haut pour cet accord (jamais une
#     case choisie au hasard : il faut qu'elle existe) — symétrique en `.max`
#     de la logique `.min` actuelle de `diag_path` ;
#   - cible < 0 -> strictement équivalent à "pas de case" -> accord neutre (déjà
#     le comportement par défaut, rien de neuf à coder ici) ;
#   - pas de case explicite au départ -> accord neutre direct sur le nom
#     transposé, aucun calcul de case.
module Transpose
