...
Line    = Struct.new(:segments, keyword_init: true)
...
class DSLParser
  ...
  # en haut du manche") — voir `GabaritEssai1.diag_path`.
  ...
  def parse_blocks(body)
    ...
    raw_blocks.each do |raw|
      ...
      blocks << Block.new(
        lines: lines.map { |l| Line.new(segments: parse_line(l)) },
  ...
  def parse_line(line)
    ...
    matches.each_with_index do |m, i|
      ...
      segments << Segment.new(chord: self.class.normalize_chord(m[1]), fret: m[2], text: line[text_start...text_end])
    end
  ...
  # Méthode de classe (toujours publique) : réutilisée telle quelle par
  # `GabaritEssai1.parse_lyr_line`.
