...
## Chantier EN COURS — SVG custom pour tablatures (2026-08-28, remplace LilyPond)
Décision Phil : remplacer LilyPond par un rendu SVG maison pour `tab:` UNIQUEMENT (`score:`/`image:` intouchés — Phil : personne n'a parlé des partitions, hors sujet). Cause des 3 défauts constatés sur Blackbird (moche, 4 mesures/système au lieu de 6, trop gros) : calibrage LilyPond approximatif (`PageBuilder::SLOT_WIDTH_PT`/`SLOT_OVERHEAD_PT` estimés à la main, `Tablator::STAFF_SIZE` fixe). Plan approuvé : `tools/tablator/tablator.rb` garde tout le parsing `.tab` (tokenize/CORDE_CASE_RE/CHORD_RE/split_into_duration_measures...), remplace juste `convert_token`/`render_measures`/`to_lilypond`/`render_svg` par un `render_tab_svg` qui dessine direct (largeur de mesure calculée EXACTE au lieu d'estimée, un seul SVG multi-système au lieu de fragments multi-fichiers — le bug `-dcrop` qui motivait la fragmentation ne s'applique plus puisqu'on ne dépend plus de LilyPond). Indicatif de métrique affiché au 1er système SEULEMENT, masqué ensuite (tranché ce tour). Répercussions : `PageBuilder.ensure_tabla_fragments`→`ensure_tabla_svg` (1 chemin, plus de liste), `measures_per_page`/SLOT_* supprimés, `Layout.build_tabla_element_v2` simplifié (plus de boucle d'empilement système par système), `tests/tools/tablator_spec.rb` à réécrire (teste `convert_token`/`to_lilypond`, API qui disparaît). Plan complet dans la conversation du 2026-08-28 si besoin de le retrouver.
